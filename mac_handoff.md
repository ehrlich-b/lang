...
### Bug 2: Effect system uses hardcoded x86-64 inline asm

**Impact**: All effect tests fail on ARM64

**Symptom**:
```
<inline asm>:1:7: error: unknown token in expression
        movq %rbp, (x8)
```

**Root cause**: `src/codegen_llvm.lang` emits x86-64 inline assembly for effect handlers:
- `movq %rbp, ...` - save frame pointer
- `movq %rsp, ...` - save stack pointer
- `leaq .Lhandler(%rip), ...` - get handler address
- `jmpq *...` - jump to handler

**Fix needed**: The effect system should use portable LLVM IR intrinsics instead of inline asm, OR have platform-specific implementations selected by LANGOS.

Relevant LLVM features to investigate:
- `llvm.frameaddress` / `llvm.stacksave` intrinsics
